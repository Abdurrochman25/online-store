...
func testMenuToManyMenuPermissions(t *testing.T) {
	...
	if err = randomize.Struct(seed, &b, menuPermissionDBTypes, false, menuPermissionColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, menuPermissionDBTypes, false, menuPermissionColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.MenuPermissions().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	a.R.MenuPermissions = nil
	if err = a.L.LoadMenuPermissions(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.MenuPermissions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testMenuToManyAddOpMenuPermissions(t *testing.T) {
	...
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, menuPermissionDBTypes, false, strmangle.SetComplement(menuPermissionPrimaryKeyColumns, menuPermissionColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*MenuPermission{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddMenuPermissions(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.MenuPermissions[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.MenuPermissions[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.MenuPermissions().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testMenuToManySetOpMenuPermissions(t *testing.T) {
	...
	err = a.SetMenuPermissions(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetMenuPermissions(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.MenuPermissions[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.MenuPermissions[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}
...
func testMenuToManyRemoveOpMenuPermissions(t *testing.T) {
	...
	err = a.AddMenuPermissions(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveMenuPermissions(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.MenuPermissions) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.MenuPermissions[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.MenuPermissions[0] != &e {
