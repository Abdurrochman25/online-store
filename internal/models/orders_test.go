...
func testOrderToOneProductUsingProduct(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadProduct(ctx, tx, false, (*[]*Order)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testOrderToOneSetOpProductUsingProduct(t *testing.T) {
	var err error
...
func testOrderToOneRemoveOpProductUsingProduct(t *testing.T) {
	var err error
...
var (
	orderDBTypes = map[string]string{`ID`: `integer`, `UserID`: `smallint`, `ProductID`: `smallint`, `Qty`: `smallint`, `OrderDetail`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `DeletedAt`: `timestamp without time zone`}
	_            = bytes.MinRead
)
