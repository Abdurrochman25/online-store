...
func testProductTagsExists(t *testing.T) {
	...
	e, err := ProductTagExists(ctx, tx, o.ID)
	if err != nil {
...
func testProductTagsFind(t *testing.T) {
	...
	productTagFound, err := FindProductTag(ctx, tx, o.ID)
	if err != nil {
		t.Error(err)
	}
...
func testProductTagToOneProductUsingProduct(t *testing.T) {
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, productTagDBTypes, true, productTagColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize ProductTag struct: %s", err)
	}
...
func testProductTagToOneRemoveOpProductUsingProduct(t *testing.T) {
	var err error
...
func testProductTagToOneRemoveOpTagUsingTag(t *testing.T) {
	var err error
...
var (
	productTagDBTypes = map[string]string{`ID`: `integer`, `ProductID`: `smallint`, `TagID`: `smallint`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `DeletedAt`: `timestamp without time zone`}
	_                 = bytes.MinRead
)
