...
func testCartToOneProductUsingProduct(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadProduct(ctx, tx, false, (*[]*Cart)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testCartToOneSetOpProductUsingProduct(t *testing.T) {
	var err error
...
func testCartToOneRemoveOpProductUsingProduct(t *testing.T) {
	var err error
...
var (
	cartDBTypes = map[string]string{`ID`: `integer`, `UserID`: `smallint`, `ProductID`: `smallint`, `Qty`: `smallint`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `DeletedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
