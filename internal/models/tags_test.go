...
func testTagToManyProductTags(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := TagSlice{&a}
	if err = a.L.LoadProductTags(ctx, tx, false, (*[]*Tag)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTagToManyAddOpProductTags(t *testing.T) {
	var err error
...
func testTagToManySetOpProductTags(t *testing.T) {
	var err error
...
func testTagToManyRemoveOpProductTags(t *testing.T) {
	var err error
...
var (
	tagDBTypes = map[string]string{`ID`: `integer`, `Name`: `character varying`, `Description`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `DeletedAt`: `timestamp without time zone`}
	_          = bytes.MinRead
)
